...
echo "Run ETL"
docker run -it --rm --name etl-runner --env THERAPEUTIC_AREA=honeur --env REGISTRY=%REGISTRY% --env ETL_IMAGE_NAME=etl-wurzburg/etl --env ETL_IMAGE_TAG=latest --env DATA_FOLDER_HOST=%DATA_FOLDER_HOST% --env DATA_FOLDER=%DATA_FOLDER% --env QA_FOLDER_HOST=%QA_FOLDER_HOST% --env QA_FOLDER_ETL=%QA_FOLDER_ETL% --env LOG_FOLDER_HOST=%LOG_FOLDER_HOST% --env LOG_FOLDER=%LOG_FOLDER_ETL% --env RUN_DQD=true -v /var/run/docker.sock:/var/run/docker.sock -v %CD%\questions.json:/script/questions.json --network feder8-net %REGISTRY%/%REPOSITORY%/%IMAGE%:%TAG%
