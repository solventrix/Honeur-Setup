...
docker pull harbor.honeur.org/ecrf/oncocologne/postgres:%VERSION%
docker pull harbor.honeur.org/ecrf/oncocologne/app:%VERSION%
docker pull harbor.honeur.org/ecrf/oncocologne/nginx:%VERSION%
...
docker run -d --name honeur_ecrf_postgres --network honeur-net --volume postgres_data:/var/lib/postgresql/data --env POSTGRES_PASSWORD=%POSTGRES_PASSWORD% --restart=always harbor.honeur.org/ecrf/oncocologne/postgres:%VERSION%
TIMEOUT 10

echo "initialize eCRF database"
docker run -d --name honeur_ecrf_app --network honeur-net --volume static_volume:/code/entrytool/assets --env OPAL_SUPER_USER_PASSWORD=%OPAL_SUPER_USER_PASSWORD% --env OPAL_DB_USER=postgres --env OPAL_DB_PASSWORD=%POSTGRES_PASSWORD% --env OPAL_DB_NAME=%DATABASE_NAME% --env OPAL_DB_HOST=honeur_ecrf_postgres --env OPAL_DB_PORT=5432 --env OPAL_FLUSH_DB=true --env OPAL_ENABLE_USER_DB=false --env OPAL_ENABLE_LDAP=false harbor.honeur.org/ecrf/oncocologne/app:%VERSION% gunicorn -b 0.0.0.0:8000 entrytool.wsgi
TIMEOUT 60
echo "create eCRF app container"
docker stop honeur_ecrf_app
docker rm honeur_ecrf_app
docker run -d --name honeur_ecrf_app --network honeur-net --volume static_volume:/code/entrytool/assets --env OPAL_SUPER_USER_PASSWORD=%OPAL_SUPER_USER_PASSWORD% --env OPAL_DB_USER=postgres --env OPAL_DB_PASSWORD=%POSTGRES_PASSWORD% --env OPAL_DB_NAME=%DATABASE_NAME% --env OPAL_DB_HOST=honeur_ecrf_postgres --env OPAL_DB_PORT=5432 --env OPAL_FLUSH_DB=false --env OPAL_ENABLE_USER_DB=false --env OPAL_ENABLE_LDAP=false --restart=always harbor.honeur.org/ecrf/oncocologne/app:%VERSION% gunicorn -b 0.0.0.0:8000 entrytool.wsgi
TIMEOUT 15

echo "create nginx container"
docker run -d --name honeur_ecrf_nginx --network honeur-net -v static_volume:/code/entrytool/assets --restart=always -p 80:80 harbor.honeur.org/ecrf/oncocologne/nginx:%VERSION%
